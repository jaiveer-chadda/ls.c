...
void d__dump(const FileStat *const fs) {
	...
	#define null " \33[91m(null)\33[m"

	#define STRUCT " \33[95mchar\33[m "
	#define CHAR " \33[95mchar\33[m "
	#define BOOL " \33[34mbool\33[m "
	#define S " \33[96m*\33[m "
	#define E " \33[96m=\33[m "

	#define OCT(fmt) "\33[94m" #fmt "\33[m"
	#define NUM(fmt) "\33[38;5;216m" #fmt "\33[m"

	#define STR "\33[92m'%s'\33[m"
	#define CHR "\33[92m'%c'\33[m"
	#define LCR "\33[92m'%lc'\33[m"

	#define PTR "\33[38;5;147m%p\33[m"

	#define IDX(idx) "\33[38;5;215m" DIMS("[") #idx DIMS("]") "\33[m"

	#define V(type) "\33[93m" #type "\33[m"
	...
	err("    "I"  "I"  "O" "V(TimeColour)" colour"E NUM(%u), fsf->times[A_TIME]->colour); } else
	ERR("    "I"  "I"  "O  null)
	...
	err("    "I"  "I"  "O" "V(TimeColour)" colour"E NUM(%u), fsf->times[M_TIME]->colour); } else
	ERR("    "I"  "I"  "O  null)
	...
	err("    "I"  "I"  "O" "V(TimeColour)" colour"E NUM(%u), fsf->times[C_TIME]->colour); } else
	ERR("    "I"  "I"  "O  null)
	...
	err("    "I"      "O" "V(TimeColour)" colour"E NUM(%u), fsf->times[B_TIME]->colour); } else
	ERR("    "I"      "O  null)
	ERR("    "I);
	...
	err("        "T" children"IDX(%d)  E PTR, i,  (void*)&fsf->children[i]);
	err("        "I" "O CHAR"\b"S"name"E STR			, fsf->children[i].name);
	ERR("        "I);
	} else {
	err("        "O" children"IDX(%d)  E PTR, i,  (void*)&fsf->children[i]);
	err("            "O CHAR"\b"S"name"E STR			, fsf->children[i].name); }
	}
	} else
	ERR("    "O  null);
}
