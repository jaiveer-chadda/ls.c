...
void printModeStr(const modestr str, const bool has_acl, const bool has_xattr) {
	...
	for (int i = 1; i < MODE_STR_LEN; i++) {
		if (DO_COLOUR()) {
			...
			if (!ESCS_ARE_EQUAL(last_esc, esc)) STRCAT_COLOUR(output, perm_colour_esc[esc], CSIR);
		}
	...
	if (has_xattr)	{ printf("%s%c", IF_COLOUR(ANSI(XATTR_COLOUR)), XATTR_CHAR); spaces-- ; }
	if (has_acl)	{ printf("%s%c", IF_COLOUR(ANSI(ACL_COLOUR)	 ), ACL_CHAR  ); spaces-- ; }
